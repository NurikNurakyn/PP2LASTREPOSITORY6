...
def message(msg, color): #сообщение когда проигрываем
...
def generate_food(snake_list): #создание еды
    while True:
        ...
        fy = round(random.randrange(100,HEIGHT-snake_position)/10.0)*10 #положение еды в координате у
...
def gameLoop():  # настройки и цикл самой игры
    ...
    fx, fy, food_weight, food_timer = generate_food(snake_length)
    while not game_over:  # пока игра не завершится
        ...
        if x1 >= WIDTH or x1 < 0 or y1 >= HEIGHT or y1 < 0:  # проверка если змейка выходит за границы окна игра заканчивается
            ...
            game_close = True
        if pygame.time.get_ticks() - food_timer > 5000:  # Если прошло 5 секунд, еда исчезает
            fx, fy, food_weight, food_timer = generate_food(snake_length)
        ...
        pygame.draw.rect(screen, (255, 255, 0), [fx, fy, food_size, food_size]) # рисует еду

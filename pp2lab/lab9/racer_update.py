...
def gameloop():
    ...
    TURBO_TIME = 0
    COINS = 0  # Чтобы знать сколько монет собралось за всю игру
    # Координаты игрока
    player_x = 160
    player_y = 520
    ...
    ONE_COIN_WEIGHT = random.randint(1, 3)
    # Игровой цикл
    ...
    while running:
        ...
        # Увеличение сложност
        speed_multiplier = min(1 + (COINS // 5) * 0.1,3)  # Машины ускоряются когда счетчик больше 5 каждый раз и до x3 максимум
        ...
        if player_rect.colliderect(coin_rect):
            COINS_COLLECTED += ONE_COIN_WEIGHT
            COINS+= ONE_COIN_WEIGHT
            coin_x = random.randint(40, SCREEN_WIDTH - 40)
            coin_y = random.randint(50, SCREEN_HEIGHT - 150)
            ONE_COIN_WEIGHT = random.randint(1, 3)
        ...
        # размер монет больше с большимими весами монет
        if ONE_COIN_WEIGHT == 1:
            pygame.draw.circle(screen, YELLOW, (coin_x, coin_y), 10)
        elif ONE_COIN_WEIGHT == 2:
            pygame.draw.circle(screen, YELLOW, (coin_x, coin_y), 13)
        elif ONE_COIN_WEIGHT == 3:
            pygame.draw.circle(screen, YELLOW, (coin_x, coin_y), 17)
